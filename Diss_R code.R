...
#see if the Salary is related with the GPA
ggplot(data_uk, aes(x=GPA, y=Salary))+geom_point()
data_uk %>% get_correlation(GPA~ Salary) #very low 
...
p6<- ggplot(data_uk, aes(x=Job_Sector, y=Salary))+
  geom_boxplot()
